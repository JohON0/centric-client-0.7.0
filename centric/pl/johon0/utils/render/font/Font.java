package centric.pl.johon0.utils.render.font;
...
public class Font implements IMinecraft {
    ...
    public void drawText(MatrixStack stack, ITextComponent text, float x, float y, float size, int alpha) {
        ...
        for (ITextComponent it : text.getSiblings()) {
            ...
            for (ITextComponent it1 : it.getSiblings()) {
                ...
                if (it1.getStyle().getColor() != null) {
                    ...
                    drawText(stack, draw, x + offset, y, ColorUtils.setAlpha(Color.WHITE.getRGB(), alpha), size);
                offset += getWidth(draw, size);
            }
            if (it.getSiblings().size() <= 1) {
                String draw = TextFormatting.getTextWithoutFormattingCodes(it.getString());

                drawText(stack, draw, x + offset, y, ColorUtils.setAlpha(it.getStyle().getColor() == null ? Color.WHITE.getRGB() : it.getStyle().getColor().getColor(), alpha), size);
                offset += getWidth(draw, size);
            }
        ...
        if (text.getSiblings().isEmpty()) {
            ...
            drawText(stack, draw, x + offset, y, ColorUtils.setAlpha(text.getStyle().getColor() == null ? Color.WHITE.getRGB() : text.getStyle().getColor().getColor(), alpha), size);
            offset += getWidth(draw, size);
        }
    }
