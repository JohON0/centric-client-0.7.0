...
@FunctionRegister(name = "RWHelper", type = Category.Misc, beta = false)
public class RWHelper extends Function {
    boolean joined;
    StopWatch stopWatch = new StopWatch();

    private final ModeListSetting s = new ModeListSetting("Функции",
            new BooleanSetting("Блокировать запрещенку", true),
    ...
    private void parseAirDrop(String name) {
        ...
        mc.player.sendChatMessage(".point add АирДроп " + x + " " + 100 + " " + z);
    }
    ...
    private void parseMascot(String name) {
        ...
        for (int i = 0; i < words.length; i++) {
            if (isInteger(words[i]) && i + 1 < words.length && isInteger(words[i + 1])) {
                ...
                mc.player.sendChatMessage(".point add Талисман " + x + " " + 100 + " " + z);
            }
        }
    }
    ...
    private void parseScrooge(String name) {
        ...
        if (words.length >= 2) {
            ...
            mc.player.sendChatMessage(".point add Скрудж " + x + " " + 100 + " " + z);
        }
    }
    ...
    private void resetCoordinatesAndRemoveWaypoints() {
        x = 0;
        z = 0;
        mc.player.sendChatMessage(".point remove АирДроп");
        mc.player.sendChatMessage(".point remove Талисман");
        mc.player.sendChatMessage(".point remove Скрудж");
    }
