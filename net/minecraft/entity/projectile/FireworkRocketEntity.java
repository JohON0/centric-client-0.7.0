        ...
        if (this.isAttachedToEntity()) {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    if (Main.getInstance().getFunctionRegistry().getElytraBooster().isState()) {
                        speed = 1.63F;
                    }

                    Vector3d vector3d;
                    if (Main.getInstance().getFunctionRegistry().getHitAura().isState() && Main.getInstance().getFunctionRegistry().getHitAura().getOptions().getValueByName("Коррекция движения").get() && this.boostedEntity instanceof ClientPlayerEntity) {
                        vector3d = this.getVectorForRotation(Main.getInstance().getFunctionRegistry().getHitAura().rotateVector.y, Main.getInstance().getFunctionRegistry().getHitAura().rotateVector.x);
                        Vector3d vector3d1 = this.boostedEntity.getMotion();
                        this.boostedEntity.setMotion(vector3d1.add(vector3d.x * 0.1 + (vector3d.x * (double)speed - vector3d1.x) * 0.5, vector3d.y * 0.1 + (vector3d.y * (double)speed - vector3d1.y) * 0.5, vector3d.z * 0.1 + (vector3d.z * (double)speed - vector3d1.z) * 0.5));
                    } else {
