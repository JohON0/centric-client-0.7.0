...
import centric.pl.events.impl.LookEvent;
...
import centric.pl.events.impl.EventMouseButtonPress;
...
public class MouseHelper {
    ...
    private void mouseButtonCallback(long handle, int button, int action, int mods) {
        if (handle == this.minecraft.getMainWindow().getHandle()) {
            ...
            if (!aboolean[0] && (this.minecraft.currentScreen == null || this.minecraft.currentScreen.passEvents)
                ...
                if (flag) {
                    Main.getInstance().onKeyPressed(-100 + i);

                    eventMouseButtonPress.setButton(i);
                    Main.getInstance().getEventBus().post(eventMouseButtonPress);
    ...
    private final LookEvent eventRotate = new LookEvent(0, 0);
    ...
    public void updatePlayerLook() {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused()) {
            ...
            if (this.minecraft.player != null) {
                ...
                eventRotate.yaw = (d2);
                eventRotate.pitch = (d3 * (double) i);

                Main.getInstance().getEventBus().post(eventRotate);

                if (!eventRotate.isCancel()) {
                    this.minecraft.player.rotateTowards(eventRotate.yaw, eventRotate.pitch);
                }
                if (eventRotate.isCancel()) {
                    eventRotate.open();
                }
