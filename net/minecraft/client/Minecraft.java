...
import centric.pl.events.impl.EventChangeWorld;
import centric.pl.events.impl.TickEvent;
import centric.pl.functions.api.FunctionRegistry;
//import centric.pl.functions.impl.combat.KillAura;
import centric.pl.functions.impl.misc.UnHook;
import centric.pl.functions.impl.player.FastPlace;
import centric.pl.functions.impl.player.ItemCooldown;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        if (s != null) {
            this.displayGuiScreen(new ConnectingScreen(selfDestruct.enabled ? new MainMenuScreen() : new MainScreen(), this, s, i));
        } else {
            this.displayGuiScreen(selfDestruct.enabled ? new MainMenuScreen() : new MainScreen());
        }
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.enabled) {
            ...
            this.mainWindow.setWindowTitle("Centric Recode | 1.16.5 Edition");
        }
    }
    ...
    public String getVersion() {
        ...
        return selfDestruct.enabled ? "Optifine 1.16.5" : this.launchedVersion;
    }
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.world == null) {
            guiScreenIn = selfDestruct.enabled ? new MainMenuScreen() : new MainScreen();
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Main.getInstance().getEventBus().post(tickEvent);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                FunctionRegistry functionRegistry = Main.getInstance().getFunctionRegistry();
                                KillAura killAura = functionRegistry.getHitAura();
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            Main.getInstance().getEventBus().post(player.tick);
    ...
    private void processKeyBinds() {
        ...
        FastPlace fastPlace = Main.getInstance().getFunctionRegistry().getFastPlace();

        if (this.gameSettings.keyBindUseItem.isKeyDown() && (this.rightClickDelayTimer == 0 || fastPlace.isState()) && !this.player.isHandActive()) {
            this.rightClickMouse();
        }
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Main.getInstance().getEventBus().post(new EventChangeWorld());
    }
