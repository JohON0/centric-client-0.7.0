...
import centric.pl.command.CommandDispatcher;
import centric.pl.command.impl.DispatchResult;
import centric.pl.functions.api.FunctionRegistry;
import centric.pl.functions.impl.misc.AntiPush;
import centric.pl.functions.impl.misc.UnHook;
import centric.pl.functions.impl.movement.AutoSprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Main.getInstance().playerOnServer) {
                Main.getInstance().playerOnServer = true;
            }

            Main.getInstance().getEventBus().post(eventUpdate);
            super.tick();
            if (this.isPassenger()) {
            ...
            Main.getInstance().getEventBus().post(new EventPostUpdate());
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Main.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Main.getInstance().getEventBus().post(eventMotion);
        ...
        Main.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Main.getInstance().getCommandDispatcher();
    ...
    public void setSprinting(boolean sprinting) {
        Main.getInstance().getEventBus().post(sprintEvent);
    ...
    public void updateEntityActionState() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (Main.getInstance().getRotationHandler().isActive()) {
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            NoSlowEvent noSlowEvent = new NoSlowEvent();
            Main.getInstance().getEventBus().post(noSlowEvent);
        ...
        if (autoSprint.isState() && !this.isSprinting() &&
                ...
                && flag4 && !this.isPotionActive(Effects.BLINDNESS)) {
    ...
    public Rotation getPreviousRotation() {
        return new Rotation(this.lastReportedYaw, this.lastReportedPitch);
    }
}
