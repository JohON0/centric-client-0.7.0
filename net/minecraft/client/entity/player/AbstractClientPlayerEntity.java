...
import centric.pl.events.impl.EventLook;
...
public abstract class AbstractClientPlayerEntity extends PlayerEntity {
    ...
    {
        UnHook unHook = Main.getInstance().getFunctionRegistry().getUnHook();
        if (this instanceof ClientPlayerEntity && !unHook.enabled) {
            return new ResourceLocation("centric/images/cape.jpg");
        }
    ...
    public Vector3d getLook(float partialTicks) {
        ...
        EventLook lookEvent = new EventLook(new VecRotation(yaw, pitch));
        Main.getInstance().getEventBus().post(lookEvent);
