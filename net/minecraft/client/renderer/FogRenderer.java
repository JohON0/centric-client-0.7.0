...
import centric.pl.functions.impl.render.World;
...
public class FogRenderer {
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier) {
        ...
        World customWorld = Main.getInstance().getFunctionRegistry().getWorld();
        ...
        if (fluidstate.isTagged(FluidTags.WATER)) {
            ...
            if (customWorld.isState() && customWorld.fog.get()) {
                final float[] colors = customWorld.clientcolorfog.get() ? (ColorUtils.IntColor.rgb(ColorUtils.reAlphaInt(HUD.getColor(100),100))) : ColorUtils.IntColor.rgb(customWorld.colorfog.get());
                red = colors[0];
                green = colors[1];
                blue = colors[2];
            } else {
    ...
    public static void setupFog(ActiveRenderInfo p_setupFog_0_, FogType p_setupFog_1_, float p_setupFog_2_, boolean p_setupFog_3_, float p_setupFog_4_) {
        ...
        if (f >= 0.0F) {
            ...
            if (fluidstate.isTagged(FluidTags.LAVA)) {
                ...
                if (customWorld.isState() && customWorld.fog.get()) {
                    f3 /= customWorld.distancefog.get();
                } else {
