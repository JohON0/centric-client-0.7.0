...
import centric.pl.events.impl.EventRotate;

import centric.pl.Main;
import centric.pl.events.impl.CameraEvent;
import centric.pl.functions.impl.misc.FreeLook;
...
public class ActiveRenderInfo {
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks) {
        ...
        EventRotate rotationEvent = new EventRotate(renderViewEntity.getYaw(partialTicks), renderViewEntity.getPitch(partialTicks));
        Main.getInstance().getEventBus().post(rotationEvent);

        if (rotationEvent.isCancel()) {
            this.setDirection(rotationEvent.yaw, rotationEvent.pitch);
        } else {
            this.setDirection(renderViewEntity.getYaw(partialTicks), renderViewEntity.getPitch(partialTicks));
        }
        ...
        if (thirdPersonIn) {
            ...
            FreeLook freeLook = Main.getInstance().getFunctionRegistry().getFreeLook();
            if (freeLook.cameraViewer.get() && freeLook.isState()) {
                this.movePosition(1, 0.0D, freeLook.position.get().intValue());
            } else {
                this.movePosition(-this.calcCameraDistance(0.0D), 0.0D, 0.0D);
            }
