...
public class Main {
    ...
    public static void main(String[] p_main_0_) {
        ...
        OptionSpec<Integer> optionspec7 = optionparser.accepts("proxyPort").withRequiredArg().defaultsTo("8080")
        ...
        OptionSpec<String> optionspec10 = optionparser.accepts("username").withRequiredArg()
                .defaultsTo("centric" + System.currentTimeMillis() % 8000); // hrahrahawerw805
        ...
        OptionSpec<Integer> optionspec14 = optionparser.accepts("width").withRequiredArg().ofType(Integer.class)
                .defaultsTo(854);
        OptionSpec<Integer> optionspec15 = optionparser.accepts("height").withRequiredArg().ofType(Integer.class)
                .defaultsTo(480);
        OptionSpec<Integer> optionspec16 = optionparser.accepts("fullscreenWidth").withRequiredArg()
                .ofType(Integer.class);
        OptionSpec<Integer> optionspec17 = optionparser.accepts("fullscreenHeight").withRequiredArg()
                .ofType(Integer.class);
        ...
        Util.func_240994_l_();
        Session session = new Session(optionspec10.value(optionset), s5, optionspec12.value(optionset),
                optionspec21.value(optionset));
        GameConfiguration gameconfiguration = new GameConfiguration(
                ...
                new GameConfiguration.GameInformation(flag1, s3, s4, flag2, flag3),
                new GameConfiguration.ServerInformation(s7, integer));
        Thread thread = new Thread("Client Shutdown Thread") {
            @Override
            public void run() {
                if (centric.pl.Main.getInstance() != null && centric.pl.Main.getInstance().getConfigStorage() != null) {
                ConfigStorage configStorage = centric.pl.Main.getInstance().getConfigStorage();
                FunctionRegistry functionRegistry = centric.pl.Main.getInstance().getFunctionRegistry();

                UnHook selfDestruct = functionRegistry.getUnHook();

                if (!selfDestruct.enabled && configStorage != null) {
                    configStorage.saveConfiguration("system");
                }
        ...
        try {
            ...
            Minecraft.fillCrashReport((LanguageManager) null, gameconfiguration.gameInfo.version, (GameSettings) null,
                    crashreport);
            ...
            thread1 = new Thread("Game thread") {
                ...
                public void run() {
                    try {
                        ...
                        net.minecraft.client.main.Main.LOGGER.error("Exception in client thread", throwable2);
                    }
                }
            };
            thread1.start();
