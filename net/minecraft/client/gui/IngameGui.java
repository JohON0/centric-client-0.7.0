...
import centric.pl.functions.impl.render.Crosshair;
import centric.pl.functions.impl.render.HUD;
import centric.pl.functions.impl.render.NoRender;
...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (eventDisplay.getType() == EventDisplay.Type.PRE) {
            Main.getInstance().getEventBus().post(eventDisplay);
        }
        ...
        if (!this.mc.gameSettings.hideGUI) {
            ...
            Main.getInstance().getEventBus().post(eventCancelOverlay);
        ...
        if (playerentity != null) {
            ...
            for (int l5 = MathHelper.ceil((f + (float) l1) / 2.0F) - 1; l5 >= 0; --l5) {
                ...
                if (playerentity.isPotionActive(Effects.POISON)) {
                    ...
                    i6 += noRender.isState() && noRender.element.getValueByName("Ёффект визера").get() ? 0 : 72;
                }
