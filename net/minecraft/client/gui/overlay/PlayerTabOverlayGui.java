...
import centric.pl.functions.impl.misc.BetterMinecraft;
...
public class PlayerTabOverlayGui extends AbstractGui {
    ...
    public void func_238523_a_(MatrixStack p_238523_1_, int p_238523_2_, Scoreboard p_238523_3_, @Nullable ScoreObjective p_238523_4_) {
        ...
        for (ScorePlayerTeam team : mc.world.getScoreboard().getTeams().stream().sorted(Comparator.comparing(Team::getName)).toList()) {
            ...
            if (namePattern.matcher(name).matches() && !team.getPrefix().getString().isEmpty() && !Main.getInstance().getFunctionRegistry().getUnHook().enabled) {
                boolean vanish = true;
        ...
        for (int i5 = 0; i5 < i4; ++i5) {
            ...
            if (i5 < list.size()) {
                ...
                if (isBetter) {
                    ...
                    float textWidth = Fonts.notoitalic[14].getWidth(String.valueOf(ping));
                    ...
                    Fonts.notoitalic[14].drawString(p_238523_1_, String.valueOf(ping), i1 + k2 - (flag ? 9 : 0) - textWidth - 2, l2 + 1.5f, color);
                } else {
